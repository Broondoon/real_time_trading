...
var _networkManagerQueue network.NetworkInterface
...
var _orderTypePriority = map[string]int{
	"MARKET": 0,
	"LIMIT":  1,
}
var _orderStatusPriority = map[string]int{
	"CANCELLED":   0,
	"COMPLETED":   1,
	"IN_PROGRESS": 2,
}
...
func InitalizeHandlers(
	...
	networkManagerQueue network.NetworkInterface,
	databaseManager databaseServiceUserManagement.DatabaseServiceInterface) {
	_databaseManager = databaseManager
	_networkManagerQueue = networkManagerQueue
	...
	for userId, responseWriter := range mapUserIds {
		...
		if transactions, ok := transactionMap[userId]; ok {
			...
			sort.SliceStable(formattedTransactions, func(i, j int) bool {
				...
				iOrderTypePriority := _orderTypePriority[formattedTransactions[i].OrderType]
				jOrderTypePriority := _orderTypePriority[formattedTransactions[j].OrderType]
				if iOrderTypePriority == jOrderTypePriority {
					iOrderStatusPriority := _orderStatusPriority[formattedTransactions[i].OrderStatus]
					jOrderStatusPriority := _orderStatusPriority[formattedTransactions[j].OrderStatus]
					if iOrderStatusPriority == jOrderStatusPriority {
						return formattedTransactions[i].Timestamp.Before(formattedTransactions[j].Timestamp)
					}
					return iOrderStatusPriority < jOrderStatusPriority
				}
				return iOrderTypePriority < jOrderTypePriority
			})
...
func executeOrderHandler(responseWriter network.ResponseWriter, data []byte, queryParams url.Values, requestType string) {
	log.Println("Handler: executeOrderHandler")
	log.Println("Handler data: ", string(data))
	var params network.MatchingEngineToExecutionJSON
	err := json.Unmarshal(data, &params)
	if err != nil {
		log.Println("Failed to unmarshal matching engine to execution JSON, error: ", err.Error())
	...
	responseWriter.Write(returnJson)
}
